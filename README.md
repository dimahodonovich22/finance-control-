...
## v4.3 — iPhone navigation & planner fix

- «+ Платёж» и «+ Мне должны» используют устойчивую delegated click-обработку.
- Настройки удалены из нижнего меню; открыть их можно нажатием на круглую иконку **F** слева сверху.
- Нижняя навигация снова состоит из 4 пунктов: Главная, Операции, Долги, План.
- CSS/JS получили версионированные имена файлов, чтобы Safari/PWA не смешивал новый HTML со старым JavaScript/CSS после обновления GitHub Pages.
- Service Worker v4.3 очищает предыдущие кэши и при наличии сети всегда предпочитает свежие файлы.
...
## v4.3 — Плановые обязательства и «Мне должны»

Добавлена вкладка **План**:

- обязательные запланированные платежи с суммой, сроком, категорией и комментарием;
- галочка **Оплачено** создаёт связанный расход и не даёт сумме потеряться в балансе;
- снятие галочки удаляет созданный автоматически расход;
- блок **Мне должны** с человеком/компанией, суммой, сроком и комментарием;
- галочка **Вернули** создаёт отдельный денежный приток вида `receivable_return`, который увеличивает доступный баланс, но не считается зарплатой/обычным доходом;
- неоплаченные платежи текущего месяца резервируются в расчёте безопасной суммы в день и месячного бюджета;
- просроченные платежи и возвраты визуально отмечаются;
- данные новых разделов входят в обычный JSON backup и автоматически мигрируют из v4.1;
- при последующем банковском импорте приложение пытается сверить автоматически созданный платёж/возврат по типу, точной сумме и дате ±3 дня, чтобы снизить риск дублей.
