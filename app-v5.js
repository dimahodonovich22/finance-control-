(()=>{
  ...
  const STORAGE_KEY='finance-control-v7';
  const PREVIOUS_KEYS=['finance-control-v6','finance-control-v5','finance-control-v4','finance-control-v3','finance-control-v2','finance-control-v1'];
  ...
  function sanitizeState(raw){
    ...
    out.transactions=(Array.isArray(raw?.transactions)?raw.transactions:[]).map(t=>{
      ...
      const allowedKinds=['income','expense','transfer','refund','debt_payment','receivable_out','receivable_return','savings_transfer'];
      ...
      return{
        ...
        origin:['scheduled_payment','receivable','debt'].includes(t.origin)?t.origin:'',
  ...
  function monthStats(){
    let income=0,lifestyleExpense=0,debtPayments=0,refunds=0,receivableOut=0,receivableReturns=0,transfers=0,savingsNet=0,allExpense=0,allDelta=0;
    ...
    return{income,lifestyleExpense,debtPayments,refunds,receivableOut,receivableReturns,transfers,savingsNet,allExpense,cashflow:income+refunds+receivableReturns-allExpense-receivableOut,allDelta};
  }
  ...
  function txIconLabel(category,type){
    ...
    const map={'Еда':'ЕД','Аренда':'ДМ','Транспорт':'ТР','Покупки':'ПК','Развлечения':'РЗ','Подписки':'ПД','Здоровье':'ЗД','Путешествия':'ПТ','Погашение долга':'ДЛ','Возврат долга мне':'↙','Дал в долг':'→','В отложенные':'→','Из отложенных':'←'};
  ...
  function txRow(t,{compact=false}={}){
    ...
    const note=[fmtDate(t.date),t.merchant||'',t.note].filter(Boolean).join(' • ');
    const amountClass=isTransfer?'transfer':(t.type==='income'?'income':'');
    return `<button class="tx-row tx-row-button ${isSavings?'savings-move-row':''}" type="button" data-tx-id="${esc(t.id)}" aria-label="Открыть операцию ${esc(t.category)} ${money2(t.amount)}">
  ...
  function renderDashboard(){
    ...
    $('savingsMonthDelta').textContent=savingsNet>0?`＋ ${money(savingsNet)} отложено за месяц`:savingsNet<0?`− ${money(Math.abs(savingsNet))} возвращено за месяц`:'Ничего не отложено за месяц';
  ...
  function openTxDetail(id){
    ...
    const kindLabel={income:'Доход',expense:'Расход',transfer:'Перевод',savings_transfer:'Перемещение в отложенные',debt_payment:'Платёж по долгу',receivable_out:'Дал в долг',receivable_return:'Вернули долг',refund:'Возврат'}[t.kind||t.type]||'Операция';
    $('txDetailBody').innerHTML=`<div class="tx-detail-amount ${t.type==='income'?'income':''}">${t.type==='income'?'+':'−'} ${money2(t.amount)}</div><div class="tx-detail-grid"><div><span>Тип</span><strong>${esc(kindLabel)}</strong></div><div><span>Категория</span><strong>${esc(t.category)}</strong></div>${t.note?`<div class="wide"><span>Комментарий</span><strong>${esc(t.note)}</strong></div>`:''}${t.importedByAI?'<div class="wide"><span>Источник</span><strong>Импортировано из выписки</strong></div>':''}</div>`;
    const special=['savings_transfer','debt_payment','receivable_out','receivable_return'].includes(t.kind||'');
    $('txDetailEdit').classList.toggle('hidden',special);
  ...
  $('txDetailEdit')?.addEventListener('click',()=>{const t=state.transactions.find(x=>x.id===activeTxDetailId);if(!t)return;closeDialog('txDetailDialog');prepTransaction(t.type,t)});
  $('txDetailDelete')?.addEventListener('click',()=>{
    const t=state.transactions.find(x=>x.id===activeTxDetailId);if(!t)return;
    if((t.kind||'')==='savings_transfer'&&t.type==='expense'){const savedAfter=state.transactions.reduce((sum,x)=>x.id===t.id||(x.kind||'')!=='savings_transfer'?sum:sum+(x.type==='expense'?x.amount:-x.amount),0);if(savedAfter<-0.005)return showToast('Сначала верни связанные деньги из отложенных')}
    ...
    bulkImportTransactions:(items=[],options={})=>{
      ...
      for(const raw of Array.isArray(items)?items:[]){
        ...
        if(matchingPlan){
          ...
          ensureNextRecurringPayment(matchingPlan);if(kind!=='transfer')importedDelta+=type==='income'?amount:-amount;if(externalId)existingIds.add(externalId);added++;continue;
        }
