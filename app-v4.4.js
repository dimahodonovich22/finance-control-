(()=>{
  ...
  const STORAGE_KEY='finance-control-v6';
  const PREVIOUS_KEYS=['finance-control-v5','finance-control-v4','finance-control-v3','finance-control-v2','finance-control-v1'];
  ...
  function sanitizeState(raw){
    ...
    out.transactions=(Array.isArray(raw?.transactions)?raw.transactions:[]).map(t=>{
      ...
      const allowedKinds=['income','expense','transfer','refund','debt_payment','receivable_return','savings_transfer'];
  ...
  function savingsBalance(){
    return Math.max(0,state.transactions.reduce((sum,t)=>{
      if((t.kind||'')!=='savings_transfer')return sum;
      return sum+(t.type==='expense'?t.amount:-t.amount);
    },0));
  }

  function monthStats(){
    let income=0,lifestyleExpense=0,debtPayments=0,refunds=0,receivableReturns=0,transfers=0,savingsNet=0,allExpense=0,allDelta=0;
    for(const t of state.transactions){
      ...
      if(kind==='savings_transfer'){
        savingsNet+=t.type==='expense'?t.amount:-t.amount;
        continue;
      }
      if(kind==='refund'){
        refunds+=t.amount;
    ...
    return{income,lifestyleExpense,debtPayments,refunds,receivableReturns,transfers,savingsNet,allExpense,cashflow:income+refunds+receivableReturns-allExpense,allDelta};
  }
  ...
  function txIconLabel(category,type){
    ...
    const map={'Еда':'ЕД','Аренда':'ДМ','Транспорт':'ТР','Покупки':'ПК','Развлечения':'РЗ','Подписки':'ПД','Здоровье':'ЗД','Путешествия':'ПТ','Погашение долга':'ДЛ','Возврат долга мне':'↙','В отложенные':'→','Из отложенных':'←'};
    return map[category]||String(category||'Р').trim().slice(0,2).toUpperCase()||'Р';
  }

  function txRow(t,{compact=false}={}){
    const isSavings=(t.kind||'')==='savings_transfer';
    const sign=isSavings?(t.type==='expense'?'→':'←'):(t.type==='income'?'+':'−');
    const note=[fmtDate(t.date),t.note].filter(Boolean).join(' • ');
    const amountClass=isSavings?'transfer':(t.type==='income'?'income':'');
    const actionMarkup=compact?'':`<div class="tx-actions">
      ${isSavings?'':`<button class="row-icon" type="button" data-edit-tx="${esc(t.id)}" aria-label="Редактировать"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="m4 16-.5 4.5L8 20l10.5-10.5-4-4Z" stroke-linejoin="round"/><path d="m13.5 6.5 4 4"/></svg></button>`}
    ...
    return `<div class="tx-row ${isSavings?'savings-move-row':''}">
      ...
      <div class="tx-side">
        <div class="tx-amount ${amountClass}">${sign} ${money2(t.amount)}</div>
        ${actionMarkup}
      </div>
    </div>`;
  }
  ...
  function renderDashboard(){
    ...
    const savingsDelta=$('savingsMonthDelta');
    const savingsNet=t.savingsNet||0;
    savingsDelta.textContent=savingsNet>0?`＋ ${money(savingsNet)} отложено за месяц`:savingsNet<0?`− ${money(Math.abs(savingsNet))} возвращено за месяц`:'Ничего не отложено за месяц';
    savingsDelta.className='trend-chip'+(savingsNet>0?' good':savingsNet<0?' bad':'');
    const capital=Math.max(0,t.totalCapital);
    const share=capital>0?clamp(t.saved/capital*100,0,100):0;
    $('savingsShare').textContent=`${share.toFixed(0)}% всех денег`;
    const goalAmount=Math.max(0,Number(state.settings.savingsGoalAmount)||0);
    const goalName=state.settings.savingsGoalName||'Цель не задана';
    const goalPct=goalAmount>0?clamp(t.saved/goalAmount*100,0,100):0;
    $('savingsGoalLabel').textContent=goalName;
    ...
    const cashFree=Math.max(0,t.available-reserve-planning.scheduledMonth);
    const budgetFree=budget>0?Math.max(0,budget-t.lifestyleExpense-planning.scheduledMonth):cashFree;
    const safeDaily=Math.max(0,Math.min(cashFree,budget>0?budgetFree:cashFree))/days;
    const hasSafeBasis=t.available!==0||budget>0||planning.scheduledMonth>0;
    $('safeDailyMetric').textContent=hasSafeBasis?money(safeDaily):'—';
    const salary=salaryCountdown();
    $('safeDailySub').textContent=hasSafeBasis
      ?(planning.scheduledMonth>0?`после планов ${money(planning.scheduledMonth)}`:(reserve>0?`после резерва ${money(reserve)}`:(salary?`до зарплаты ${salary.days} дн.`:`${days} дн. до конца месяца`)))
      :'задай стартовый баланс';
  ...
  function renderCategoryChart(){
    const monthExpenses=state.transactions.filter(t=>t.type==='expense'&&!['transfer','savings_transfer','debt_payment'].includes(t.kind||'expense')&&t.category!=='Погашение долга'&&isThisMonth(t.date));
    const sums=new Map();
  ...
  function renderTransactions(){
    ...
    if(currentTxFilter==='income')rows=rows.filter(t=>t.type==='income'&&!['transfer','savings_transfer'].includes(t.kind||''));
    if(currentTxFilter==='expense')rows=rows.filter(t=>t.type==='expense'&&!['transfer','savings_transfer','debt_payment'].includes(t.kind||'expense')&&t.category!=='Погашение долга');
  ...
  document.body.addEventListener('click',e=>{
    ...
    const delTx=e.target.closest('[data-del-tx]');if(delTx){
      ...
      if(t){
        if((t.kind||'')==='savings_transfer'&&t.type==='expense'){
          const savedAfter=state.transactions.reduce((sum,x)=>{
            if(x.id===t.id||(x.kind||'')!=='savings_transfer')return sum;
            return sum+(x.type==='expense'?x.amount:-x.amount);
          },0);
          if(savedAfter<-0.005){showToast('Сначала верни связанные деньги из отложенных');return}
        }
  ...
  function prepSavingsTransfer(direction='to_savings'){
    const t=totals();
    const toSavings=direction==='to_savings';
    $('savingsTransferForm').reset();
    $('savingsTransferDirection').value=direction;
    $('savingsTransferDate').value=today();
    $('savingsTransferTitle').textContent=toSavings?'Отложить деньги':'Вернуть в доступные';
    $('savingsTransferSubtitle').textContent=toSavings
      ?`Доступно сейчас ${money2(t.available)}. Перевод не считается расходом.`
      :`Отложено ${money2(t.saved)}. Возврат не считается доходом.`;
    $('savingsTransferSubmit').textContent=toSavings?'Отложить':'Вернуть';
    openDialog('savingsTransferDialog');
    setTimeout(()=>$('savingsTransferAmount').focus(),80);
  }

  $('saveMoneyBtn')?.addEventListener('click',()=>prepSavingsTransfer('to_savings'));
  $('returnMoneyBtn')?.addEventListener('click',()=>{
    if(totals().saved<=0.005)return showToast('В отложенных пока нет денег');
    prepSavingsTransfer('from_savings');
  });
  $('savingsGoalBtn')?.addEventListener('click',()=>{
    $('savingsGoalName').value=state.settings.savingsGoalName||'';
    $('savingsGoalAmount').value=state.settings.savingsGoalAmount||'';
    openDialog('savingsGoalDialog');
    setTimeout(()=>$('savingsGoalName').focus(),80);
  });

  $('savingsTransferForm')?.addEventListener('submit',e=>{
    e.preventDefault();
    const direction=$('savingsTransferDirection').value==='from_savings'?'from_savings':'to_savings';
    const amount=parseDecimalInput($('savingsTransferAmount').value);
    if(!(amount>0))return showToast('Укажи сумму');
    const t=totals();
    if(direction==='to_savings'&&amount>t.available+0.005)return showToast(`Доступно только ${money2(Math.max(0,t.available))}`);
    if(direction==='from_savings'&&amount>t.saved+0.005)return showToast(`Отложено только ${money2(t.saved)}`);
    state.transactions.push({
      id:uid(),
      type:direction==='to_savings'?'expense':'income',
      kind:'savings_transfer',
      amount,
      category:direction==='to_savings'?'В отложенные':'Из отложенных',
      date:$('savingsTransferDate').value||today(),
      note:$('savingsTransferNote').value.trim()||(direction==='to_savings'?'Перевод в отложенные':'Возврат в доступные'),
      merchant:'Отложенные деньги',
      externalId:'',
      importedByAI:false,
      origin:'',
      originId:'',
    ...
    persist(direction==='to_savings'?`Отложено ${money2(amount)}`:`Возвращено ${money2(amount)} в доступные`);
  });
  ...
  initWalletCarousel();
  render();
  if('serviceWorker' in navigator&&location.protocol.startsWith('http'))navigator.serviceWorker.register('./sw.js?v=4.4').catch(()=>{});
})();
